...
public class MovimentacaoDao {
	...
	public List<Movimentacao> pesquisa(Conta conta, TipoMovimentacao tipoMovimentacao, Integer mes){
		...
		if (conta.getId() != null) {
			conjunction = 
					builder.and(conjunction,
							builder.equal(m.<Conta>get("conta"), conta));
		}
	
		if (mes != null && mes != 0) {
			Expression<Integer> expression =
					builder.function("month", Integer.class, m.<Calendar> get("data"));
			conjunction =
		...
		if (tipoMovimentacao != null) {
			conjunction =
					builder.and(conjunction,
							builder.equal(
									m.<TipoMovimentacao>get("tipoMovimentacao"),
																			tipoMovimentacao));
		}
	...
	public List<Movimentacao> listaComCategorias() {
		return manager.createQuery(
				"select distinct m from Movimentacao m "
						+ "left join fetch m.categorias").getResultList();
	...
	public List<ValorPorMesEAno> listaMesesComMovimentacoes(Conta conta,
		...
		TypedQuery<ValorPorMesEAno> query = manager.createQuery(jpql,
